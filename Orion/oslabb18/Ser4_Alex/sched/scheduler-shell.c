...
static void createNode (struct Node *node);
static void deleteNode (struct Node *node);
static void childProc (pid_t *p, char *argv);
static void nextScheduled(void);
...
{
	struct Node *curcur = head->next;
	while (curcur != tail){
		printf("The Process with PID: %ld is in the queue\n",curcur->myid);
		curcur = curcur->next;
	}
...
{
	// stamatane ola otan kanw kill!FIXME 
	// menei na vrw th diergasia poy pethane kai na th diagrapsw FIXME
	kill(getpid()+(pid_t)id,SIGKILL);
	return 1;
}
...
sched_create_task(char *executable)
{
	...
	if (stat(executable, &sb) == 0 && sb.st_mode & S_IXUSR){
		createNode(tail->prev);
		cur->p = safe_malloc(sizeof(pid_t));
		childProc(&*(cur->p),executable);
	}
	//assert(0 && "Please fill me!");
...
int main(int argc, char *argv[])
{
	...
	tail->prev = head;
	cur = head;
	createNode(head->next);
	cur->p = &shell_p;
	cur->myid = (long int)shell_p % (long int)getpid();
...
static void childProc (pid_t *p, char *argv){
	*p = fork();
	...
	if (*p > 0){
		cur->myid = (long int)*p  % (long int)getpid();
	}
}
